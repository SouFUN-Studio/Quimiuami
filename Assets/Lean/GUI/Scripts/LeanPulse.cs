...
namespace Lean.Gui
{
	...
	public class LeanPulse : MonoBehaviour
	{
		...
		public LeanTime Timing { set { timing = value; } get { return timing; } } [SerializeField] private LeanTime timing;

		/// <summary>This allows you to perform an animation when this UI element pulses.
		/// You can create a new transition GameObject by right clicking the transition name, and selecting <b>Create</b>.
		/// For example, the <b>Play Sound Transition (LeanPlaySound)</b> component can be used to play a pulse sound.</summary>
		...
		protected virtual void Update()
		{
			...
			if (LeanTransition.GetTimingAbs(finalUpdate) == LeanTime.Update)
			{
		...
		protected virtual void LateUpdate()
		{
			...
			if (LeanTransition.GetTimingAbs(finalUpdate) == LeanTime.LateUpdate)
			{
				UpdatePulse(finalUpdate > 0 ? Time.deltaTime : Time.unscaledDeltaTime);
			}
		}
		...
		protected virtual void FixedUpdate()
		{
			...
			if (LeanTransition.GetTimingAbs(finalUpdate) == LeanTime.FixedUpdate)
			{
